...
Doctor Diagnostic Notes: ${assessment}`;
    ...
    try {
      ...
      if (data.success && data.soapNote) {
        ...
        const provenance = note.ai_provenance || null;
        ...
        // An outage must never read like a generated note — that is how a
        // fabricated diagnosis ends up signed into a patient's record.
        setAiFailed(provenance?.method === 'fallback');
        setAiMessage(
          provenance?.method === 'fallback'
            ? 'The AI models could not be reached, so NOTHING here was generated by AI. Write the diagnosis and plan yourself. Check /api/ai/health?live=1 for the reason.'
            : `AI draft complete — ${provenance?.models_consulted?.length || 1} model(s) consulted. Review and edit every line before saving.`
        );
      } else {
        setAiFailed(true);
        setAiProvenance(null);
        setAiMessage(`The AI request failed, so nothing was generated. ${data.error || ''}`.trim());
      }
    } catch (err) {
      console.warn('[AI SOAP FAIL]', err);
      setAiFailed(true);
      setAiProvenance(null);
      setAiMessage(`Could not reach the AI service, so nothing was generated. (${(err as Error).message})`);
    } finally {
      ...
      <div className="app-modal app-modal-lg">
        <div className="card shadow-lg border-0 rounded-4 overflow-hidden">
          ...
          <div className="card-body p-4 bg-light overflow-y-auto" style={{ maxHeight: '78vh' }}>
            ...
            {aiMessage && (
              <div
                className={`alert d-flex align-items-start justify-content-between gap-2 p-2 mb-3 ${
                  aiFailed ? 'alert-danger' : 'alert-info'
                }`}
                role="alert"
              >
                <span className="small d-flex align-items-start gap-2">
                  <i className={`bi mt-1 ${aiFailed ? 'bi-exclamation-octagon-fill' : 'bi-stars'}`} />
                  <span>{aiMessage}</span>
                </span>
